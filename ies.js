...
let iesLoaderModulePromise = null;

function requestIESLoader() {
  if (!iesLoaderModulePromise) {
    iesLoaderModulePromise = import(
      "https://unpkg.com/three-ies-loader@1.0.0/build/IESLoader.js"
    ).catch((error) => {
      console.warn("Unable to import three-ies-loader, falling back to manual parser.", error);
      return null;
    });
  }
  return iesLoaderModulePromise;
}
